...
function message_left($data,$row)
{
	...
	<div></div>
		...
		<b>$row->username</b>
		<div id='msgl'>$data->message</div>";

		if($data->files != "" && file_exists($data->files)){
			$a .= "<img src='$data->files' style='width:100%;cursor:pointer;' onclick='image_show(event)' /> <br>";
		}
		$a .= "<span style='font-size:8px;color:black;padding-left:28px;'>".date("jS M Y H:i:s a",strtotime($data->date))."<span>
...
function message_right($data,$row)
{
	...
	$a .= "</div>
		...
		<b>$row->username</b>
		<div id='msgr'>$data->message</div>";

		if($data->files != "" && file_exists($data->files)){
			$a .= "<img src='$data->files' style='width:100%;cursor:pointer;' onclick='image_show(event)' /> <br>";
		}
		$a .= "<span style='font-size:8px;color:white;padding-right:28px;'>".date("jS M Y H:i:s a",strtotime($data->date))."<span>
...
function message_controls()
{
	...
	<div style='display:flex;width:100%;height:40px;'>
		...
		<input id='message_text' onkeyup='enter_pressed(event)' style='flex:6;border:solid thin #ccc;border-bottom:none;font-size:14px;padding:4px;' type='text' placeHolder='tulis pesan'/>
